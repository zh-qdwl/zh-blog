...
### ~~A3. 侧栏入场错峰只覆盖 5 个 widget 中的 2 个~~（已修）

加第 6 个 widget（小程序入口）时一并解决了，做法和下面设想的 `--n` 不同：
`motion.css` 补到 `:nth-child(5)`，第 6 个及以后用**开区间** `:nth-child(n + 6)`
统一封顶 400ms —— 新 widget 会自动落进这一档，不会再漏。

同时修掉了 `layout.css` 里那条更危险的：窄屏排序原本**逐个列出**要下沉的 widget，
漏一个就会以 `order:0` 排到内容上方、把文章列表挤下去。改成「全部下沉 + 头像卡
一个例外」，新 widget 默认就在正确位置。
